...
2. **`agentcockpit agent`** — host daemon running on the developer's machine. Connects outbound to the relay (works on locked-down devices — no inbound ports needed). Manages PTY sessions. Listens on `/tmp/agentcockpit-agent.sock` for hook shim calls.
...
| Package | Purpose |
|---|---|
| `cmd/agentcockpit/` | Cobra CLI entry point. Subcommands: `serve`, `connect`, `agent`, `hook`, `hooks`, `session`, `user` |
...
Hosts self-register via an invite flow: browser generates a short-lived invite token (`POST /api/hosts/invite`), CLI claims it (`POST /api/hosts/claim`). On first `agentcockpit agent` run, the agent auto-registers if no config exists.
